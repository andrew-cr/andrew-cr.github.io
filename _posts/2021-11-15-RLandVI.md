...
The ELBO maximization approach works well when we have a fixed number of observations $$y_{1:T}$$. However, what happens if we continually recieve new observations and wish to perform inference after we recieve each one i.e. we would like to approximate $$p(x_{1:T} \vert y_{1:T})$$ and then $$p(x_{1:T+1} \vert y_{1:T+1})$$ and then $$p(x_{1:T+2} \vert y_{1:T+2})$$ and so on.
...
The variational distribution $$q(x_{1:T-1})$$ contains only fixed factors and so the term inside the expectation does not contribute directly to objective gradients for the new factors $$q(x_T)$$ and $$q(x_{T-1} \vert x_T)$$. However, this term depends on $$x_{T-1}$$ *which does depend on the new factors* because $$x_{T-1}$$ is being sampled from $$x_{T-1} \sim q(x_T) q(x_{T-1} \vert x_T)$$. Therefore, this term will contribute to the gradient of the objective and cannot be ignored. We need a way to avoid having to directly calculate this term as it requires stepping all the way back through from $$T-1$$ to $$1$$ every time step and we want an objective that is quick to calculate at every time step and its cost to evaluate should not grow with time $$T$$.
...
We are now finally ready to see how this can be used to solve our problem from before where we had a term in our sequential variational inference objective that depended on the new variational factors through $$x_{T-1}$$ but had a large number of terms from $$T-$$ all the way back to $$1$$.
...
We first define our state from RL to be our hidden state in our model $$s_t = x_t$$. We then also define our action $$a_t$$ to be the hidden state in our model but at the previous time step $$a_t = x_{t-1}$$. This seems rather strange but it allows our policy $$\pi_\phi(a_t \vert s_t)$$ to correspond to one of our variational factors $$\pi_\phi(a_t \vert s_t) = q(x_{t-1} \vert x_t)$$. Due to this strange definition of states and actions, our state transition function becomes a delta function $$P(s_t \vert s_{t+1}, a_{t+1}) = \delta (s_t = a_{t+1})$$. We can now define our new `reward'

$$ r(s_t, a_t) = r(x_{t}, x_{t-1}) = \log \frac{f(x_t\vert x_{t-1}) g(y_t \vert x_t) q(x_{t-1})} {q(x_t) q(x_{t-1} \vert x_t) } $$

# With these definitions, our tracjectory distrubtion also becomes our full variational approximation

# $$ p_\phi(\tau) = q(x_T) q(x_{T-1} \vert x_T) \prod_{t={T-1}}^1 q(x_{t-1} \vert x_t)$$
...
$$ V_{t-1} (x_{t-1}) = \mathbb{E}_{q(x_{1:T-2} \vert x_{T-1})} \left[ \log \frac{p(x_{1:T-1}, y_{1:T-1})}{q(x_{1:T-1})}\right]$$

And it still follows the same forward recursion from before

$$ V_{t}(x_t) = \mathbb{E}_{q(x_{t-1} \vert x_t )} \left[  \log \frac{f(x_t\vert x_{t-1}) g(y_t \vert x_t) q(x_{t-1})} {q(x_t) q(x_{t-1} \vert x_t) }+ V_{t-1}(x_{t-1}) \right]$$
